...
本页并列呈现两个视角：左侧说明具身智能的能力分工与长期积累方向；右侧说明这些能力在Loco-Manipulation中如何组织和落地。

前半部分回答Loco-Manip任务由哪些能力共同完成，后半部分回答第一次学习时应该按什么顺序建立机器人模型、反馈控制、仿真训练和失败分析能力。

## 本页导航

[系统能力栈](#系统能力栈) · [训练与更新闭环](#训练与更新闭环) · [六条资料路线](#知识库中的六条路线) · [支撑资料](#支撑资料入口) · [三条项目路线](#三条可执行项目路线) · [共同基础](#共同基础) · [最终作品验收](#最终作品验收) · [按起点进入](#根据自己的起点进入) · [继续查资料](#继续查资料)

## 系统能力栈

左侧以System 1 / System 0区分上层智能与快速身体能力，是本库的分析约定，不是行业统一编号或固定部署架构。右侧的三种系统组织方式可以混合使用，不代表线性升级关系。
...
<td valign="top"><pre>
      ...
      全身协调 / 平衡接触 / 偏差修正 / 扰动恢复
        ...
        +-----------------+-----------------+
        |                 |                 |
 感知与状态估计      模型与硬件协同    安全与控制权协调
 环境/本体/接触      策略/控制/执行    约束/仲裁/保护
  提供可靠反馈        提高执行精度      管理运行风险
</pre></td>
<td valign="top"><pre>
                  Loco-Manipulation
      移动 · 全身协调 · 物体交互 · 连续任务
...
走路/抓取/搬运切换   腿腰手联合控制    目标/动作块/计划
        ...
        +-----------------+-----------------+
        |                 |                 |
 感知与状态估计      全身运动基座      任务规划与预测
视觉/本体/力触觉   平衡/跟踪/恢复/接触  记忆/世界模型
    共同需要          共同需要          按需接入
</pre></td>
</tr></tbody>
</table>

图中连线表示能力之间的联系，不是单向流水线：上层下发目标，身体控制结合当前状态执行，并将执行结果反馈给上层。

**行业积累方向：** 上层能力可基于通用多模态模型或自研具身基座，通过领域后训练适配任务；长期积累依赖真实场景数据，以及采集、仿真、训练、评测和部署基础设施。数据与更新闭环见下一节，不与在线控制链混画。

**左图按功能分工，不按项目划界：** System 0描述快速身体控制闭环，不限定为某个基础模型。它可以采用模型式优化控制、学习策略或二者结合；上层可以下发任务目标、技能、轨迹或动作块，具体接口随系统设计而定。两层可以分开实现，也可以联合训练或整合到统一策略中，不要求串联两个独立模型。

这一归纳参考了[模型式全身控制](https://ai.stanford.edu/manips/publications/pdfs/Sentis_2006_ICRA.pdf)、[运动跟踪与上层调用接口](https://nvlabs.github.io/GR00T-WholeBodyControl/references/index.html)和[视觉分层世界模型控制](https://arxiv.org/abs/2405.18418)等不同实现，属于本库的系统分析，不是这些工作的统一架构声明。

> 快速反应不必等待上层推理，但仍需感知、训练与安全约束；AMP运动先验本身不等于视觉避障能力。世界预测也可以服务低层控制，不专属于System 1。

**如何验收精度：** 例如指定坐标系下移动10厘米，或将物体放到指定位置，应联合检查目标表达、状态估计、策略输出、控制跟踪与硬件标定，并记录误差、扰动条件、成功率和恢复能力。精度是整个闭环的结果，不单由模型或硬件负责。
...
下面这些页面不构成新的技术路线，而是六条路线共同使用的数据、基础知识和工程选型入口。

| 支撑资料 | 主要解决的问题 |
| --- | --- |
| [具身智能数据集](../数据集/README.md) | 按来源、模态、本体依赖、动作表示、许可和下游用途查找数据。 |
| [具身训练数据来源与本体依赖](../数据集/具身训练数据来源与本体依赖.md) | 理解人体视频、遥操作、仿真、真机数据怎样经过转换进入训练闭环。 |
| [Ego第一人称数据采集设备选型](../数据集/Ego第一人称数据采集设备选型.md) | 判断第一人称设备实际交付什么信号，以及距离可训练数据还缺哪些环节。 |
| [基础书籍与学习资料](基础书籍与学习资料.md) | 根据当前问题补机器人学、动力学、规划、状态估计和强化学习基础。 |
| [强化学习开发者必备开源资料](../强化学习开发者必备开源资料/README.md) | 选择环境接口、算法框架、机器人仿真和足式训练项目。 |
...
建议从[Humanoid-Gym](../论文与项目/论文逐篇解读/P015.md)及其[开源实现](https://github.com/roboterax/humanoid-gym)建立训练、MuJoCo验证和部署接口，再按问题阅读[AMP](../论文与项目/论文逐篇解读/P022.md)、[DreamWaQ](../论文与项目/论文逐篇解读/P125.md)或[BFM-Zero](../论文与项目/论文逐篇解读/P080.md)。阅读时要分清任务奖励、感知表征、先验奖励和技能变量分别修改哪一层。
